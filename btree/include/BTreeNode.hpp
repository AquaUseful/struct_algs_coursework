...
namespace btree {
  ...
  public:
    BTreeNode(const size_t order, bool leaf = true) : m_order {order}, m_leaf {leaf} {
    ...
    BTreeNode(BTreeNode& sibling, value_iterator_t first, node_ptr_iterator_t children_first) : BTreeNode(sibling.m_order, sibling.m_leaf) {
    ...
    BTreeNode(const size_t order, value_t median, node_ptr_t left, node_ptr_t right) : BTreeNode(order, false) {
    ...
    value_const_reference_t front() const {
      return m_values.front();
    }
    value_const_reference_t back() const {
    ...
    node_ptr_const_iterator_t children_cend() const {
      return m_children.cend();
    }

    node_ptr_iterator_t children_begin() {
      return m_children.begin();
    }
    node_ptr_iterator_t children_end() {
  ...
  private:
    ...
    node_ptr_iterator_t children_midpoint() {
      return std::next(children_begin(), std::distance(children_begin(), children_end()) / 2);
    }
    ...
    bool underfilled() const {
      return m_values.size() < ((m_order - 1) / 2);
    }
    bool minfilled() const {
      return m_values.size() <= ((m_order - 1) / 2);
    }
    bool overfilled() const {
      return m_values.size() >= (m_order);
    }
    ...
    insert_result_t split() {
      ...
      node_ptr_t sibling = std::make_unique<node_t>(*this, std::next(median), ch_midpoint);
      ...
      if (!(*sibling)->minfilled()) {
        ...
        *val = borow_res.value;
        if (borow_res.node != nullptr) {
          (*chld)->m_children.insert((*chld)->children_begin(), std::move(borow_res.node));
        }
        return true;
      }
      return false;
    ...
    void merge(value_t mid, node_ptr_t right) {
      ...
      if (!m_leaf) {
        ...
        std::move(right->children_begin(), right->children_end(), right_child(ipos));
      }
